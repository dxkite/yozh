# Benchmark: astro-runtime (goja) vs Node.js SSR

本报告对比 **astro-runtime**（goja/sobek 引擎）与**原生 Node.js V8** 在相同 Astro SSR
应用下的吞吐量、延迟、镜像大小与资源消耗。

> **说明**：astro-runtime 曾同时支持 QuickJS（WASM）与 goja 两种引擎，本报告早前版本包含
...
| 最小镜像体积 | astro-runtime 单静态二进制，显著小于包含 `node_modules` 的 Node.js 镜像 |
